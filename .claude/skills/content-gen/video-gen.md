...
- ❌ 不支持 `--resolution`
- ✅ 支持 `--duration` / `--ar` / `--seed`
- 参考视频通过 `--ref-video <url>` 传入，**可多次使用传入多个视频**
...
# 4. 提交（Step 2）——只用 --prompt，不用 --promptfile
...
- 示例：`镜头1.mp4` 已存在 → 新文件自动命名为 `镜头1-v2.mp4`
- 示例：`开场.mp4` 已存在 → 新文件自动命名为 `开场-v2.mp4`
...
- **环境音与音效**：默认应包含与画面匹配的环境音（脚步声、风声、物体碰撞声等）和必要音效。
- **背景音乐**：默认不包含任何背景音乐、BGM 或配乐。除非用户明确要求，才在 prompt 中指定音乐风格。
- **人声对白**：如果视频含有人物对话/独白，必须在 prompt 正文中完整写出台词。

这条规则必须写入每个视频 prompt 文件的声音规则章节，确保默认行为一致。
...
- **环境音与音效**：默认应包含与画面匹配的环境音（脚步声、风声、物体碰撞声等）和必要音效。
- **背景音乐**：默认不包含任何背景音乐、BGM 或配乐。如果用户明确要求，才在 prompt 中指定音乐风格。
- **人声对白**：如果视频含有人物对话/独白，必须在 prompt 中完整写出台词（见下方视频提示词要求）。

这条规则写入每个视频 prompt 文件的声音规则章节，确保默认行为一致。

# 视频提示词

**重要：如果视频包含人物对话/独白，必须在 prompt 正文中完整写出所有台词，并标注每句台词对应的时间点和表情/动作。不要把台词丢了。**

Prompt 正文分散在各分层章节中（参考图、视觉风格、人物外貌、环境描述、镜头设计/人物动作、光影构图等），**不需要单独的"完整提示词"汇总章节**。各章节内容共同构成提交给模型的完整生成指令。
```
...
2. 调用 submit，**用 `--prompt` 传入 prompt 正文**，或用 `--promptfile` 直接读取 prompt 文件：

**Image-to-Video 示例**：
```bash
npx -y bun ${SKILL_DIR}/scripts/video/main.ts --provider newapi submit \
  --prompt "一只胖橘猫战士，右手持锯齿短刀、左手持木质圆角盾，站在浅色瓷砖地面上，面向镜头露出憨态可掬的微笑" \
  --ref-image 项目-漫剧-x/3-素材/角色/主角/形象.jpg \
  --duration 5 --ar 16:9
...
**多参考素材示例**（多张图片 + 多个视频 + 音频同时传入）：
```bash
npx -y bun ${SKILL_DIR}/scripts/video/main.ts --provider newapi submit \
  --prompt "角色 A 和角色 B 在赛博朋克街道上对峙，背景要有霓虹灯招牌" \
  --ref-image "项目-漫剧-x/3-素材/角色/角色A/形象.jpg" \
  --ref-image "项目-漫剧-x/3-素材/角色/角色B/形象.jpg" \
  --ref-video "https://xgy1.oss-cn-wulanchabu.aliyuncs.com/ootd/xxxx_动作参考1.mp4" \
  --ref-video "https://xgy1.oss-cn-wulanchabu.aliyuncs.com/ootd/xxxx_镜头参考2.mp4" \
  --ref-audio "项目-漫剧-x/3-素材/音效/背景音.mp3" \
  --duration 5 --ar 16:9
# Output: task_xxxxxxxx
```
...
npx -y bun ${SKILL_DIR}/scripts/video/main.ts submit --prompt "一只猫在花园里弹钢琴" --ref-image cat.jpg
...
| `--prompt <text>`, `-p` | Prompt text (inline) |
...
| `--duration <seconds>` | Video duration in seconds, **only supports 5/10/15** (default: 5) |
